...
namespace VehicleTracking.IdentityServer.Stores {
    public class ResourceStore : IResourceStore {
        ...
        public async Task<Resources> GetAllResourcesAsync() {
            ...
            id4Resources.ToList().ForEach(s => {
                s.UserClaims = new List<Model.IdentityClaim> { new Model.IdentityClaim { Type = "I add this from code IdentityClaim" } };
            });
            var id4ApiResources = await _apiResourceRepository.GetAllAsync();
            id4ApiResources.ToList().ForEach(s => {
                s.UserClaims = new List<Model.ApiResourceClaim> { new Model.ApiResourceClaim { Type = "I add this from code ApiResourceClaim" } };
            });
            var id4ApiScope = await _identityApiScopeRepository.GetAllAsync();
            id4ApiScope.ToList().ForEach(s => {
                s.UserClaims = new List<Model.ApiScopeClaim> { new Model.ApiScopeClaim { Type = "I add this from code ApiScopeClaim" } };
            });
